   ...
   private static void createMap(Map<Character, Integer> map) {
      ...
      for (String token: tokens){ 
         for (int i = 0; i < token.length(); i++) {
            Character character = Character.toLowerCase(token.charAt(i)); // get lowercase character
                     
            // if the map contains the character
            if (map.containsKey(character)) { // is character in map?
               int count = map.get(character); // get current count
               map.put(character, count + 1); // increment count   
            } 
            else {
               map.put(character, 1); // add new character with a count of 1 to map
            } 
         } 
      }
   } 
